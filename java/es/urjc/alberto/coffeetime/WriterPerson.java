...
public class WriterPerson implements Runnable{
    ...
    public WriterPerson(String name, String destin, String message, View v, Activity acti){
        this.name = name;
        this.destin = destin;
    ...
    public void run(){
        ...
        try {
            ...
            odata.writeInt(1);
            Integer length = destin.length();
            odata.writeInt(length);
            odata.write(destin.getBytes("UTF-8"));
            length = name.length();
            odata.writeInt(length);
            ...
            if(ok == 1){
                ...
                String toas = "User destination not exist!";
                v.post(new showToast(acti, toas));
            }else if(ok == 3){
                String toas = "You can't write yourself!";
                v.post(new showToast(acti, toas));
            }else if(ok == 0){
